...
def get_options():
    ...
    parser.add_option("--loadseg_rdb_dir",
                      default=os.path.join(os.environ['SKA'], 'data', 'arc',
                                           'iFOT_events', 'load_segment'),
                      help="directory with iFOT rdb files of load segments")
...
def get_built_load(run_load, dbh=None):
    ...
    if built is None:
        ...
        if match_like is None:
            raise ValueError(
                "Load name %s is in unknown form, expects /CL\d{3}:\d{4}/" %
                run_load['load_segment'])
        like_load = match_like.group(1)
        built_query = ("""select * from tl_built_loads
                          where load_segment like '%s%s%s'
                          and year = %d
                          order by sumfile_modtime desc"""
...
def get_processing(built, dbh=None):
    ...
    if processed is None:
        raise ValueError("Unable to find processing for built file %s",
                         built['file'])
    return processed
...
def get_replan_dir(replan_seg, replan_year, dbh=None):
    ...
    :param replan_seg: replan commands src string (like C123:1029 or C014_0293)
    ...
    if match_like is None:
        raise ValueError(
            "Replan load seg %s is in unknown form, expects /C\d{3}?\d{4}/" %
            replan_seg)
...
def weeks_for_load(run_load, dbh=None, test=False):
    ...
    The tl_processing table contains the information from the processing
    summary:
    ...
    :param test: test mode option to allow the routine to continue
                 on missing history
    ...
    if processed['replan'] and processed['bcf_cmd_count'] > 0:
        if processed['replan_cmds'] is None:
            raise ValueError(
                "Replan/ReOpen load: %s without replan_cmd source load!" %
                run_load['load_segment'])
        # if load is completely contained in processing
        # (earlier load interrupted in week)
        ...
        # if processing covers the end but not the beginning
        # (this one was interruped)
        if (run_load['datestart'] < processed['processing_tstart']
            and run_load['datestop'] <= processed['processing_tstop']):
            replan_dir = get_replan_dir(processed['replan_cmds'],
                                        run_load['year'],
                                        dbh=dbh)
            log.warn("TIMELINES WARN: %s,%s is replan/reopen, " %
                     (run_load['year'], run_load['load_segment'])
                     + "using %s dir for imported cmds" % replan_dir)
            # the end
        ...
        if ((DateTime(run_load['datestart']).mxDateTime
             >= DateTime(built['first_cmd_time']).mxDateTime)
            & (DateTime(run_load['datestop']).mxDateTime
             <= DateTime(built['last_cmd_time']).mxDateTime)):
            match['incomplete'] = 0
        # append even if incomplete
        match_load_pieces.append(match.copy())

    timelines = sorted(match_load_pieces,
                       key=lambda k: (k['datestart'], k['load_segment_id']))
    return timelines


def get_ref_timelines(datestart, dbh=None, n=10):
    ref_timelines = []
    pre_query = ("""select * from timelines
                    where datestart <= '%s'
    ...
    for cnt in xrange(0, n):
        try:
            ...
            if (cnt == 0):
                log.warn(
                    "TIMELINES WARN: no timelines found before current insert")
            else:
                log.warn(
                    "TIMELINES WARN: only found %i of %i " % (cnt - 1, 10)
                    + "timelines before current insert")
...
def update_timelines_db(loads=None, dbh=None, dryrun=False, test=False):
    """
    Given a list of load segments this routine determines the
    timelines (mission planning weeks and loads etc) over the loads
    and inserts new timelines into the aca timelines db table.
    ...
    for run_timeline in timelines[i_diff:]:
        time_length = (DateTime(run_timeline['datestop']).mxDateTime
                       - DateTime(run_timeline['datestart']).mxDateTime)
        if time_length.minutes < 60:
            log.warn("TIMELINES WARN: short timeline at %s, %d minutes" %
                     (run_timeline['datestart'],
                      time_length.minutes))
    ...
    max_id = (dbh.fetchone('SELECT max(id) AS max_id FROM timelines')['max_id']
              or 0)
...
def rdb_to_db_schema(orig_ifot_loads):
    ...
    for orig_load in orig_ifot_loads:
        ...
        loads.append(load)
    names = ('load_segment', 'year',
             'datestart', 'datestop',
             'load_scs', 'fixed_by_hand')
...
def check_load_overlap(loads):
    ...
    if (any(sep_times > max_sep)):
        for load_idx in np.flatnonzero(sep_times > max_sep):
            log.warn(
                "LOAD_SEG WARN: Loads %s %s separated by more that %i hours"
...
def clear_timeline(id, dbh=None, dryrun=False):
    ...
    :param dbh: database handle for cmd_fltpars,
                cmd_intpars, cmds, and timelines tables
...
def clear_rel_timelines(load_segments, dbh=None, dryrun=False):
    """
    Remove timelines related to (fk constrained to) load_segments
    from the timelines db
    ...
    for load in load_segments:
        db_timelines = dbh.fetchall("""SELECT * from timelines
                                       where load_segment_id = %i"""
                                    % (load['id']))

        if len(db_timelines) > 0:
            if (any(db_timelines['fixed_by_hand'])):
                fbh = db_timelines['fixed_by_hand'] == 1
                for timeline in db_timelines[fbh]:
                    log.warn("LOAD_SEG WARN: updating timelines across %i "
                             % timeline['id']
                             + "which is fixed_by_hand")
...
def get_last_timeline(dbh=None):
    ...
    timeline = dbh.fetchone("""SELECT * from timelines where datestart
                                  = (select max(datestart) from timelines)""")
    return timeline
...
def find_load_seg_changes(wants, haves, exclude=[]):
    ...
    # Use explicit increment for i_diff so that it gets set to the
    ...
    if i_diff == 0:
        raise ValueError(
            "Unexpected mismatch at first database entry in range")
...
def update_loads_db(ifot_loads, dbh=None, test=False, dryrun=False):
    ...
    # if no overlap on time range, check for an empty table
    # (which should be OK)
    if len(db_loads) == 0:
        count_all_db_loads = dbh.fetchall(
            "select count(*) as all_count from load_segments")
        count_all = count_all_db_loads[0]['all_count']
        if count_all == 0:
            log.warn("LOAD_SEG_WARN: load_segment table is empty")
        else:
            raise ValueError(
                "No overlap in database for load segment interval")
        to_delete = []
        to_insert = ifot_loads
    else:
        to_delete, to_insert = find_load_seg_changes(ifot_loads,
                                                     db_loads,
                                                     exclude=['id'])
    ...
    later_loads = dbh.fetchall(
        "select * from load_segments where datestart >= '%s'" %
    ...
    # check for overlap in the loads and log warnings
    check_load_overlap(ifot_loads)

    max_id = (dbh.fetchone(
            "SELECT max(id) AS max_id FROM load_segments")['max_id']
              or 0)

    for load in to_insert:
        log.debug('LOAD_SEG DEBUG: inserting load')
        insert_string = "\t %s %d %s %s %d" % (load['load_segment'],
                                               load['year'],
                                               load['datestart'],
                                               load['datestop'],
                                               load['load_scs'])
...
def main(loadseg_rdb_dir, dryrun=False, test=False,
         dbi='sqlite', server='db_base.db3', database=None, user=None,
         verbose=False):
    """
    ...
    Details:
    Reads most recent RDB file from arc data iFOT load_segments directory.
    ...
    as an update to the load_segments table must happen prior to
    get_timelines()

    """

    dbh = DBI(dbi=dbi, server=server, database=database, user=user,
              verbose=verbose)
    ...
    if len(ifot_loads):
        # make any scripted edits to the tables of parsed files
        # to override directory mapping
