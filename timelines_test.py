...
# Fixing the database for the command collision bugs in March 2012 revealed another 
# bug whereby 2 timelines are not reinserted for 2 new load segments because there
# is already 1 matching timeline.  Test added to prevend recurrence.
    ...
    for want_entry, have_entry in izip(first_timelines, second_timelines):
        if any(have_entry[x] != want_entry[x] for x in match_cols):
            assert False
                ...
                # there should still be 2 timelines for CL078:0401
#    assert len(np.flatnonzero(
#            timelines['datestart'] == '2012:078:04:56:59.586')) == 2
